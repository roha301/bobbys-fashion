...
import { useHomeDecorProduct, useHomeDecorProducts } from '../hooks/useHomeDecor'
import { useWishlist } from '../context/WishlistContext'
import { homeDecorApi } from '../api/homeDecorClient'

const INR = new Intl.NumberFormat('en-IN', { style: 'currency', currency: 'INR', maximumFractionDigits: 0 })

export default function HomeDecorProductDetails() {
  const { id } = useParams()
  const { loading, data: product } = useHomeDecorProduct(id)
  const [activeImage, setActiveImage] = useState(0)
  const [size, setSize] = useState(null)
  const [color, setColor] = useState(null)
  const wishlist = useWishlist()

  const related = useHomeDecorProducts(product ? { category: product.category } : {})
  ...
  if (!product) {
    return (
      <div className="mx-auto max-w-7xl px-5 py-24 text-center">
        ...
        <Link to="/home-decor" className="mt-4 inline-block text-sm text-[var(--color-gold-dark)] hover:underline">Back to Home Decor</Link>
      </div>
    )
  }
      ...
      <div className="grid grid-cols-1 gap-10 lg:grid-cols-2">
        ...
        <div>
          <div className="aspect-[3/4] overflow-hidden rounded-2xl bg-[var(--color-paper-dim)] p-4 flex items-center justify-center">
            <img src={images[activeImage]} alt={product.name} referrerPolicy="no-referrer" className="max-h-full max-w-full object-contain rounded-xl shadow-sm" />
          </div>
        ...
        <div>
          ...
          {product.sizes?.length > 0 && (
            <div className="mt-6">
              <p className="mb-2 text-xs font-medium text-[var(--color-ink)]">Available Dimensions/Sizes</p>
          ...
          {product.colors?.length > 0 && (
            <div className="mt-5">
              <p className="mb-2 text-xs font-medium text-[var(--color-ink)]">Available styles/colors</p>
              <div className="flex flex-wrap gap-2">
          ...
          <div className="mt-8 flex gap-3">
            <a
              ...
              onClick={() => homeDecorApi.registerClick(product.id).catch(() => {})}
        ...
        <div className="mt-16">
          ...
          <div className="grid grid-cols-2 gap-5 sm:grid-cols-3 lg:grid-cols-4">
            {related.data
              ...
              .slice(0, 4)
              .map((p, i) => <HomeDecorProductCard key={p.id} product={p} index={i} />)}
          </div>
        </div>
      )}
    </div>
